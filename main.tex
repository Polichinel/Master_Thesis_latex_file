...
In my final predictive framework I use eight features; all pertaining directly to the temporal and spatial patterns of conflict. These eight features will all be derived from two functions estimated for each grid cells time line using the train set and Gaussian processes. Four features from each function. The two functions will respectively be an estimate of the pattern of conflict magnitude and the pattern of conflict exposure given the (training) time line of each grid cell. Given each cell, I will extrapolate these two functions into the future years, contained in the test set. The value of each of the two function at any given year constitutes a feature; so does the slope of the function that year, the accelaration of the function that year and the total mass under the function given all years. This constitutes the eight features used in the final predictive framework. It is the construction of these eight features, which is my objective in this section.\par

% IN what order:
I will start by estimating and extrapolating the function pertaining to conflict magnitude. Then I will move on to estimating and extrapolating the function pertaining to conflict exposure. Finally I will derive the slope. accelaration and mass corresponding to each function.\par

% The two functions requires the measures to be estimated against.
To estimate the function pertaining to conflict magnitude I simply use conflict fatalities logged as my target ($y_{cm}$) and the years in my train set as the sole feature ($x_{year}$). The mean function is simply $0$ and the covariance function is the squared exponential function. The relevant priors can be found in the appendix.\par
...
% hyper parameters 
First I estimate the hyper parameters...
...
% Drawing samples.
I introduce $x_{new}$, with is just a vector of alls years. Train and test alike. Using the estimated hyperparemeters and the data contained in the train set I now draw GPS for each time line acorss all years included in $x_new$. A sample of estimated function can be seen in [FIG].
