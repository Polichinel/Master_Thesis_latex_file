...
The challenge now is to estimate the temporal and spatial patterns of conflict in each grid cell at all years in included in the train set. These patterns will then be extracted into future years corresponding to those of the test set. From these elongated times lines I will then derive the corresponding slope and acceleration pertaining to each year along with the total conflict mass inhabiting each time line. This yields eight features which will then be combined in my final predictive framework to estimate the probabilities of conflict in each individual grid cell given the test years from 2013 through 2017. Lastly I evaluate how well my predictions fit with the actual conflicts present throughout the test years.\par

%The implementation is divided into two steps.
As such the following section is divided into two subsections. The first one focuses on using Gaussian processes to capture the patterns of conflict and extrapolate these into future years. The second subsection focuses on using the eight features in the final predictive framework based on xgboost and undersampling. Together these two subsections constitutes the two layers of my predictive framework.\par
